...
for (i in 1:nrow(all_sim_result)) {
  result <- Simulation_for_Error(B = B, n = 100000, betaE = all_sim_result$betaE[i], betaG = 0,
                                       betaGE = 0, maf = all_sim_result$maf[i])
...
# alpha = 5e-3
# i = 6
# load(file = paste0("betaE_", all_sim_result$betaE[i], "_maf", all_sim_result$maf[i], "_test_size_n100000.rda"))
