...
simulate_genotype <- function(n, p) {
  ...
  # Fisher orthogonal encoding
  # GA = (0, 1, 2) respectively
  df$GA <- rowSums(genotype_matrix)
  # Fisher orthogonal encoding
  # GD = -p/q, 1, -q/p for (0, 1, 2) respectively
  df$GD <- ifelse(df$GA == 0, -p/q,
                  ifelse(df$GA == 1, 1, -q/p))
...
replication_once <- function(seed, maf_vec){
  ...
  for (maf in maf_vec) {
    # Simulate a genotype dataframe
    df_try <- simulate_genotype(300000, maf)
    df_all <- simulate_phenotype(df = df_try, binary = TRUE)
...
# result_once_orthogonal <- data.frame(
#   maf = numeric(),
#   joint_pval = numeric(),
#   add_pval = numeric(),
#   dom_pval = numeric()
...
# B <- 50
# for (i in 1:B) {
#   result_once_orthogonal <- rbind(result_once_orthogonal, replication_once(i, seq(0.05, 0.5, by = 0.05)))
# }
# 
# # save the result
# save(result_once_orthogonal, file = "result_once_orthogonal.RData")
...
library(tidyverse)
# load the result
load("result_once_orthogonal.RData")

# summarize the mean, upper and lower bounds of the p-values
summary_joint <- result_once_orthogonal %>%
  group_by(maf) %>%
  summarise(
...
summary_add <- result_once_orthogonal %>%
  group_by(maf) %>%
  summarise(
...
summary_dom <- result_once_orthogonal %>%
  group_by(maf) %>%
  summarise(
...
# plot
png("joint_test_vs_additive_and_dominant_ortho.png", width = 450, height = 450)
...
png("joint_test_vs_additive_and_dominant_correlation_ortho.png", width = 450, height = 450)
par(mfrow = c(1, 1), mar = c(5, 5, 2, 2))
